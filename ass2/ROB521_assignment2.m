...
for i=2:numodom
    d_t = t_odom(i) - t_odom(i-1);
    theta = theta_odom(i-1) + omega_odom(i) * d_t;
    theta_odom(i) = theta;
    x_odom(i) = x_odom(i-1) + v_odom(i) * cos(theta_odom(i-1)) * d_t;
    y_odom(i) = y_odom(i-1) + v_odom(i) * sin(theta_odom(i-1)) * d_t;
end
theta_odom = mod(theta_odom + 3.14, 2*3.14) - 3.14; 
...
for n=1:100
    ...
    for i=2:numodom
        d_t = t_odom(i) - t_odom(i-1);
        theta_odom(i) = theta_odom(i-1) + omega_odom(i) * d_t;
        x_odom(i) = x_odom(i-1) + v_odom(i) * cos(theta_odom(i-1)) * d_t;
        y_odom(i) = y_odom(i-1) + v_odom(i) * sin(theta_odom(i-1)) * d_t;
    end
    % ------end of your wheel odometry algorithm------- 
...
for n=1:2
    ...
    for i=1:size(t_laser,1)
        ...
        % Step 1: Loop over all laser scans and find the position in the
        % local frame
        if abs(omega_interp(i)) > omega_threshold
            continue
        end
        x_start = x_interp(i);
        y_start = y_interp(i);
        vehicle_location = zeros(2, 1);
        vehicle_location(1, 1) = x_start;
        vehicle_location(2, 1) = y_start;
        theta = theta_interp(i);

        % Construct the 2x2 rotation matrix from the vehicle frame to
        % the inertial frame
        % We assume that theta is the angle from the inertial frame to the
        % vehicle frame
        C = [
            [cos(theta), -sin(theta)]
            [sin(theta), cos(theta)]
        ...
        step_ranges = y_laser(i, :);
        local_xs = cos_angles .* step_ranges;
        local_ys = sin_angles .* step_ranges;
        sensor_f_points = [local_xs; local_ys];

        % Account for the positioning of the sensor
        vehicle_f_points = sensor_f_points - [0.1; 0];


        % Rotate the points into the inertial frame
        inertial_r_points = C * vehicle_f_points;
        % Transform the points by 
        intertial_f_points = inertial_r_points + vehicle_location;

        % Add the new inertial frame points to all_points
        % all_points = [all_points intertial_f_points];

        % Plot the new points
        if n==2
            % For noise free odometry, plot in black
            color = 'b';
        else
            color = 'r';
        end

        scatter(intertial_f_points(1, :), intertial_f_points(2, :), 5, color, "filled")
